...
Production-ассеты не создаём заранее. Сначала проверяем core loop. Когда конкретный ассет становится нужен для следующего этапа, готовим отдельное ТЗ для AI-дизайнера.
...
Утверждённый 16:9 gameplay-макет остаётся в рабочем контексте проекта и **не хранится бинарным файлом в Git**.

Он является главным визуальным референсом по:
...
Только после того, как gameplay prototype подтверждает механику и соответствующий ассет реально нужен в разработке:
...
Для каждого такого шага сначала готовится отдельный prompt/brief для AI-дизайнера.
