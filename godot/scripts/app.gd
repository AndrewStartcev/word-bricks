...
# Application shell around gameplay. Menus/modals use normal Control nodes instead
# of custom _draw(), while the board keeps the existing gameplay renderer.
...
const COL_BG: Color = Color("071426")
...
const COL_GOLD: Color = Color("ffd45a")
const COL_GREEN: Color = Color("69de87")
const COL_DANGER: Color = Color("ef6b72")
...
func _show_main_menu() -> void:
	...
	var panel: PanelContainer = _panel(Vector2(520.0, 560.0), COL_PANEL)
	_center_control(panel, Vector2(80.0, 0.0))
	screen_layer.add_child(panel)

	var margin: MarginContainer = _margin(54, 48, 54, 48)
	panel.add_child(margin)
	var column: VBoxContainer = VBoxContainer.new()
	column.add_theme_constant_override("separation", 18)
	margin.add_child(column)

	var title: Label = _label("СЛОВОПАД", 58, COL_TEXT, HORIZONTAL_ALIGNMENT_CENTER)
	column.add_child(title)
	var subtitle: Label = _label("Лес · 5 слов", 20, COL_GREEN, HORIZONTAL_ALIGNMENT_CENTER)
	...
	var status: Label = _label(status_text, 16, status_color, HORIZONTAL_ALIGNMENT_CENTER)
...
func _show_level_select() -> void:
	...
	var top_panel: PanelContainer = _panel(Vector2(0.0, 0.0), Color(0.015, 0.045, 0.095, 0.90))
	top_panel.position = Vector2(90.0, 54.0)
	top_panel.size = Vector2(1420.0, 86.0)
	...
	var progress_label: Label = _label("1 / 6", 18, COL_MUTED, HORIZONTAL_ALIGNMENT_RIGHT)
	...
	var levels: Array[Dictionary] = [
		{"title":"Лес", "subtitle":"5 слов", "open":true, "icon":ICON_CHAPTER},
		{"title":"Море", "subtitle":"Скоро", "open":false, "icon":ICON_LOCK},
		{"title":"Город", "subtitle":"Скоро", "open":false, "icon":ICON_LOCK},
		{"title":"Еда", "subtitle":"Скоро", "open":false, "icon":ICON_LOCK},
		{"title":"Космос", "subtitle":"Скоро", "open":false, "icon":ICON_LOCK},
		{"title":"Сказки", "subtitle":"Скоро", "open":false, "icon":ICON_LOCK}
	]

	for index in range(levels.size()):
		var data: Dictionary = levels[index]
		var card: Button = _level_card(index + 1, String(data["title"]), String(data["subtitle"]), bool(data["open"]), data["icon"] as Texture2D)
		if bool(data["open"]):
...
func _start_game() -> void:
	...
	# Real Control buttons cover the prototype's painted buttons and intercept clicks.
...
func _show_pause_modal() -> void:
	...
	_clear_modal()
	current_modal = "pause"
	game.set("manual_paused", true)
	if game.has_method("_sync_audio_pause"):
		game.call("_sync_audio_pause")

	var panel: PanelContainer = _modal_panel(Vector2(520.0, 530.0))
	...
	var menu_button: Button = _button("В меню", "ghost", ICON_HOME, 54.0)
...
func _show_result_modal(victory: bool) -> void:
	...
	owl.expand_mode = TextureRect.EXPAND_IGNORE_SIZE
	owl.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
	owl.custom_minimum_size = Vector2(180.0, 145.0)
	owl.mouse_filter = Control.MOUSE_FILTER_IGNORE
	...
	var words_count: int = int((game.get("completed_words") as Array).size())
	var small_text: String = "%d / 5 слов" % words_count
	column.add_child(_label(small_text, 18, COL_MUTED, HORIZONTAL_ALIGNMENT_CENTER))
	column.add_child(_modal_spacer(6.0))
...
func _settings_row(title: String, icon: Texture2D, is_music: bool) -> Control:
	...
	toggle.text = "Вкл"
	toggle.button_pressed = music_enabled if is_music else sfx_enabled
...
func _modal_panel(panel_size: Vector2) -> PanelContainer:
	...
	shade.color = Color(0.002, 0.01, 0.025, 0.74)
	_full_rect(shade)
	shade.mouse_filter = Control.MOUSE_FILTER_STOP
	modal_layer.add_child(shade)

	var panel: PanelContainer = _panel(panel_size, Color(0.014, 0.05, 0.105, 0.985), true)
...
func _modal_column(panel: PanelContainer) -> VBoxContainer:
	var margin: MarginContainer = _margin(48, 36, 48, 36)
	panel.add_child(margin)
	var column: VBoxContainer = VBoxContainer.new()
...
func _stat_card(label_text: String, value_text: String, icon: Texture2D) -> Control:
	...
	row.add_child(col)
	col.add_child(_label(label_text, 13, COL_MUTED, HORIZONTAL_ALIGNMENT_LEFT))
	col.add_child(_label(value_text, 20, COL_TEXT, HORIZONTAL_ALIGNMENT_LEFT))
	return panel


func _level_card(number: int, title: String, subtitle: String, unlocked: bool, icon: Texture2D) -> Button:
...
func _button(text: String, variant: String, icon: Texture2D = null, height: float = 60.0) -> Button:
	...
	button.add_theme_stylebox_override("focus", _style_box(Color(0, 0, 0, 0), Color(0.55, 0.82, 1.0, 0.9), 10, false))
	return button
